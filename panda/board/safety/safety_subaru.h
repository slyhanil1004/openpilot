...
const CanMsg SUBARU_TX_MSGS[] = {{0x122, 0, 8}, {0x221, 0, 8}, {0x321, 0, 8}, {0x322, 0, 8}, {0x40, 2, 8}, {0x139, 2, 8}, {0x576, 0, 8}}; // ES_LKAS_State
...
static int subaru_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (GET_BUS(to_push) == 2U)) {
    int addr = GET_ADDR(to_push);
    if (addr == 0x576) {
      bool lkas_pressed = (GET_BYTE(to_push, 2) & 0x02) > 0; // LKAS_ACTIVE signal
