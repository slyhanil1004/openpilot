...
OnroadScreenOff::OnroadScreenOff() : AbstractControl("Driving Screen Off", "Turn off the device screen or reduce brightness to protect the screen after driving starts. It automatically brightens or turns on when a touch or event occurs.", "../assets/offroad/icon_shell.png")
{
...
OnroadScreenOffBrightness::OnroadScreenOffBrightness() : AbstractControl("Driving Screen Off Brightness (%)", "When using the Driving Screen Off feature, the brightness is reduced according to the automatic brightness ratio.", "../assets/offroad/icon_shell.png") {
...
BrightnessControl::BrightnessControl() : AbstractControl("Brightness Control (%)", "Manually adjusts the brightness of the screen.", "../assets/offroad/icon_shell.png") {
...
ChangeFork::ChangeFork() : ButtonControl("Change Fork/Branch", "", "Change to another openpilot fork. You can change it by entering ID/repository/branch.") {

  QObject::connect(this, &ButtonControl::clicked, [=]() {
    if (text() == "CHANGE") {
      //QString userId = InputDialog::getText("1) Enter the GitHub username.", this);
      QString userId = "sunnyhaibin";
      if (userId.length() > 0) {
        getUserID(userId);
        //QString repoId = InputDialog::getText("2) Enter the repository.", this);
        QString repoId = "openpilot";
        if (repoId.length() > 0) {
          getRepoID(repoId);
          QString branchId = InputDialog::getText("Enter the branch name.", this);
          if (branchId.length() > 0) {
            getBranchID(branchId);
            githubBranch = branchId;
            QString changeFork1 = QString::fromStdString("Change Fork/Branch. It will take some time to complete. Are you sure you want to proceed?\n") + QString::fromStdString("https://github.com/") + githubUsername + QString::fromStdString("/") + githubRepo + QString::fromStdString(".git\n") + QString::fromStdString("Branch: ") + githubBranch;
            if (ConfirmationDialog::confirm(changeFork1.toStdString().c_str(), this)) {
              setText("DONE");
              setEnabled(true);
              QString time_format = "yyyyMMddHHmmss";
              QDateTime curDateTime = QDateTime::currentDateTime();
              QString cdt = curDateTime.toString(time_format);
              QString changeFork2 = "mv /data/openpilot /data/openpilot_" + cdt;
              QString changeFork3 = "git clone -b " + githubBranch + " --depth 1 --single-branch https://github.com/" + githubUsername + "/" + githubRepo + ".git /data/openpilot";
              QProcess::execute("pkill -f thermald");
              QProcess::execute(changeFork2);
              QProcess::execute(changeFork3);
              Hardware::reboot();
            }
          }
        }
...
void ChangeFork::refresh() {
  setText("CHANGE");
  setEnabled(true);
}

void ChangeFork::getUserID(const QString &userId) {
  ...
  QObject::connect(request, &HttpRequest::failedResponse, [=] {
    ConfirmationDialog::alert(userId + " The GitHub username does not exist. Return to the input window, press the cancel button, and try again from the beginning.", this);
    refresh();
    request->deleteLater();
  });
  QObject::connect(request, &HttpRequest::timeoutResponse, [=] {
    ConfirmationDialog::alert("The requested has timed out.", this);
    refresh();
    request->deleteLater();
  });
...
void ChangeFork::getRepoID(const QString &repoId) {
  HttpRequest *request = new HttpRequest(this, false);
  QObject::connect(request, &HttpRequest::receivedResponse, [=](const QString &resp) {
    if (!resp.isEmpty()) {
      githubRepo = repoId;
  ...
  request->sendRequest("https://github.com/" + githubUsername + "/" + repoId);
}

void ChangeFork::getBranchID(const QString &branchId) {
  HttpRequest *request = new HttpRequest(this, false);
  QObject::connect(request, &HttpRequest::receivedResponse, [=](const QString &resp) {
    if (!resp.isEmpty()) {
      githubBranch = branchId;
  ...
  request->sendRequest("https://github.com/" + githubUsername + "/" + githubRepo + "/tree/" + branchId);
}